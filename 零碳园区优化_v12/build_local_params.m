...
p.carbonQuotaMode = get_data_field(data, 'carbonQuotaMode', 'baseline-scenario');
fixedCarbonQuota_kg = get_data_field(data, 'fixedCarbonQuota_kg', []);
useFixedCarbonQuota = strcmpi(char(p.carbonQuotaMode), 'fixed-baseline') && ~isempty(fixedCarbonQuota_kg);
...
p.QtradeMax_tCO2 = get_data_field(data, 'QtradeMax_tCO2', 1.6);
p.QmarketMax_tCO2 = get_data_field(data, 'QmarketMax_tCO2', 1e4);
p.carbonTradeAllPeriods = get_data_field(data, 'carbonTradeAllPeriods', true);
p.carbonBalanceMode = get_data_field(data, 'carbonBalanceMode', 'period');
...
p.PloadFixed = get_row_field(data, 'PloadFixed', data.Pload, i);
p.PdrShiftBase = get_row_field(data, 'PdrShiftBase', zeros(data.N,data.T), i);
p.PdrShiftMax = get_row_field(data, 'PdrShiftMax', p.PdrShiftBase, i);
p.PdrCutEmax = get_row_field(data, 'PdrCutEmax', zeros(data.N,data.T), i);
p.Hload  = data.Hload(i,:);
p.HdrCutMax = get_row_field(data, 'HdrCutMax', zeros(data.N,data.T), i);
p.H2load = data.H2load(i,:);
p.H2drCutMax = get_row_field(data, 'H2drCutMax', zeros(data.N,data.T), i);
p.pfTan = get_indexed_field(data, 'pfTan', data.Qbase(i,:) ./ max(data.Pbase(i,:), 1e-6), i);
p.cDRShiftE = get_indexed_field(data, 'cDRShiftE', 0, i);
p.cDRCutE = get_indexed_field(data, 'cDRCutE', 0, i);
p.cDRCutH = get_indexed_field(data, 'cDRCutH', 0, i);
p.cDRCutH2 = get_indexed_field(data, 'cDRCutH2', 0, i);
...
p.QwindMax = get_indexed_field(data, 'QwindMax', zeros(data.N,1), i);
p.QesMax = data.QesMax(i);
p.lambdaQpv = data.lambdaQpv(i);
p.lambdaQwind = get_indexed_field(data, 'lambdaQwind', zeros(data.N,1), i);
...
function row = get_row_field(s, name, defaultVal, i)
if isfield(s, name) && ~isempty(s.(name))
    raw = s.(name);
else
    raw = defaultVal;
