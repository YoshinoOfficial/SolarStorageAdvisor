...
Ppv = zeros(N,T);
Pwind = zeros(N,T);
pvCap = [1.60; 2.00; 1.50];         % MW, 光伏装机容量(差异化)
windCap = [0.00; 1.20; 0];       % MW, 风电装机容量
PchMax  = [0.90; 1.40; 1.00];       % MW
PdisMax = [0.80; 1.20; 0.90];       % MW
Emax    = 4.0 * PdisMax;            % MWh
...
% Electric load = fixed load + shiftable load - curtailable reduction.
PdrShiftRatio = [0.08; 0.12; 0.10];
PdrCutRatioE  = [0.06; 0.04; 0.08];
PdrShiftBase  = repmat(PdrShiftRatio,1,T) .* Pload;
PloadFixed    = Pload - PdrShiftBase;
PdrShiftMax   = 2.00 * PdrShiftBase + 0.02;
PdrCutEmax    = repmat(PdrCutRatioE,1,T) .* Pload;

% Heat and hydrogen mainly use curtailable demand response.
HdrCutRatio  = [0.08; 0.06; 0.08];
H2drCutRatio = [0.10; 0.08; 0.10];
cDRShiftE = 40  * ones(N,1);     % Yuan/MWh, shift discomfort compensation
cDRCutE   = 650 * ones(N,1);     % Yuan/MWh, electric load curtailment compensation
cDRCutH   = 380 * ones(N,1);     % Yuan/MWh_th, heat load curtailment compensation
cDRCutH2  = 120 * ones(N,1);     % Yuan/kg, hydrogen load curtailment compensation
...
HdrCutMax  = repmat(HdrCutRatio,1,T) .* Hload;
H2drCutMax = repmat(H2drCutRatio,1,T) .* H2load;
...
lambdaH2Short = 300 * ones(N,1);      % 元/kg
lambdaQpv     = 5.0 * ones(N,1);      % 元/(MVAr^2*h)
lambdaQwind   = 5.0 * ones(N,1);      % 元/(MVAr^2*h)
...
QinjMin    = min(Qbase,[],2) - QpvMax(:) - QwindMax(:) - QesMax(:) - 0.05;
...
% Planning-layer compatible renewable data.
% These fields preserve unit-capacity DDRE profiles and nominal installed
% capacities, so capacity-planning models can replace pvCap/windCap by
% decision variables without losing the original weather profiles.
data.pvCap0 = pvCap(:);
data.windCap0 = windCap(:);
data.ddrePpvPU = ddrePpvPU;
data.ddrePwindPU = ddrePwindPU;
data.pvProfilePU = ddrePpvPU;
data.windProfilePU = ddrePwindPU;
...
% 负荷
data.Pload = Pload;
data.PloadFixed = PloadFixed;
data.PdrShiftBase = PdrShiftBase;
data.PdrShiftMax = PdrShiftMax;
data.PdrCutEmax = PdrCutEmax;
data.Hload = Hload;
data.HdrCutMax = HdrCutMax;
data.H2load = H2load;
data.H2drCutMax = H2drCutMax;
data.cDRShiftE = cDRShiftE(:);
data.cDRCutE = cDRCutE(:);
data.cDRCutH = cDRCutH(:);
data.cDRCutH2 = cDRCutH2(:);
...
data.lambdaQwind = lambdaQwind(:);
data.lambdaQes = lambdaQes(:);

data.QpvMax = QpvMax(:);
data.QwindMax = QwindMax(:);
data.QesMax = QesMax(:);
