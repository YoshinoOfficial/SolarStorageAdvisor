...
%   - carbon allowance trading variables QaBuy/QaSell/Qtrade/QaUnused: tCO2
...
lambdaQwind = i_get_field(data, 'lambdaQwind', zeros(N,1));
    ...
    for i = 1:N
        ...
        for t = 1:T
            ...
            if enableCarbonQuota && isfield(si, 'QaBuy') && isfield(si, 'QaSell')
                carbonTradingCost = carbonTradingCost + carbonBuyPrice*si.QaBuy(t) - carbonSellPrice*si.QaSell(t);
            end
            ...
            rampCost      = rampCost      + cRampEb(i)        * (i_get_sol_field(si,'RupEb',t) + i_get_sol_field(si,'RdnEb',t));
            rampCost      = rampCost      + cRampElec(i)      * (i_get_sol_field(si,'RupElec',t) + i_get_sol_field(si,'RdnElec',t));
            rampCost      = rampCost      + cRampFc(i)        * (i_get_sol_field(si,'RupFc',t) + i_get_sol_field(si,'RdnFc',t));
            rampCost      = rampCost      + cRampComp(i)      * (i_get_sol_field(si,'RupComp',t) + i_get_sol_field(si,'RdnComp',t));
            ...
            demandResponseCost = demandResponseCost + cDRShiftE(i)*i_get_sol_field(si,'PdrShiftDev',t)*dt + ...
                cDRCutE(i)*i_get_sol_field(si,'PdrCutE',t)*dt + cDRCutH(i)*i_get_sol_field(si,'HdrCut',t)*dt + ...
                cDRCutH2(i)*i_get_sol_field(si,'H2drCut',t)*dt;
            qSupportCost  = qSupportCost  + (data.lambdaQpv(i) * si.Qpv(t)^2 + lambdaQwind(i) * i_get_sol_field(si,'Qwind',t)^2 + data.lambdaQes(i) * si.Qes(t)^2) * dt;
        end
    end
else
    for i = 1:N
        for t = 1:T
            ...
            if enableCarbonQuota && isfield(sol, 'QaBuy') && isfield(sol, 'QaSell')
                carbonTradingCost = carbonTradingCost + carbonBuyPrice*sol.QaBuy(i,t) - carbonSellPrice*sol.QaSell(i,t);
            end
            ...
            rampCost      = rampCost      + cRampEb(i)        * (i_get_matrix_field(sol,'RupEb',i,t) + i_get_matrix_field(sol,'RdnEb',i,t));
            rampCost      = rampCost      + cRampElec(i)      * (i_get_matrix_field(sol,'RupElec',i,t) + i_get_matrix_field(sol,'RdnElec',i,t));
            rampCost      = rampCost      + cRampFc(i)        * (i_get_matrix_field(sol,'RupFc',i,t) + i_get_matrix_field(sol,'RdnFc',i,t));
            rampCost      = rampCost      + cRampComp(i)      * (i_get_matrix_field(sol,'RupComp',i,t) + i_get_matrix_field(sol,'RdnComp',i,t));
            ...
            demandResponseCost = demandResponseCost + cDRShiftE(i)*i_get_matrix_field(sol,'PdrShiftDev',i,t)*dt + ...
                cDRCutE(i)*i_get_matrix_field(sol,'PdrCutE',i,t)*dt + cDRCutH(i)*i_get_matrix_field(sol,'HdrCut',i,t)*dt + ...
                cDRCutH2(i)*i_get_matrix_field(sol,'H2drCut',i,t)*dt;
            qSupportCost  = qSupportCost  + (data.lambdaQpv(i) * sol.Qpv(i,t)^2 + lambdaQwind(i) * i_get_matrix_field(sol,'Qwind',i,t)^2 + data.lambdaQes(i) * sol.Qes(i,t)^2) * dt;
        end
    end
end
...
obj = gridCost + carbonCost + gasCost + gasCarbonCost + carbonTradingCost + hdumpCost + chpOMCost + ...
      ...
      h2ShortCost + demandResponseCost + qSupportCost;
...
parts = struct(...
    ...
    'demandResponseCost', demandResponseCost, ...
...
function val = i_get_sol_field(s, name, t)
if isfield(s, name) && ~isempty(s.(name))
    val = s.(name)(t);
else
    val = 0;
...
function val = i_get_matrix_field(s, name, i, t)
if isfield(s, name) && ~isempty(s.(name))
    val = s.(name)(i,t);
else
    val = 0;
