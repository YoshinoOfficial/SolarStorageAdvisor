...
% Submission plotting script for zero-carbon campus multi-energy model
    ...
    for k = 1:nScen
        ...
        draw_hydrogen_profiles(R, D, scenNames{k}, outDir);
...
function draw_objective_comparison(allResults, scenNames, outDir)
    ...
    for k = 1:nScen
        ...
        if ~isempty(sol)
            admmObj(k) = get_scalar_safe(sol,'recoveredGlobalObjective');
            if admmObj(k) == 0
                admmObj(k) = get_scalar_safe(sol,'obj');
            end
            if admmObj(k) == 0
                admmObj(k) = get_scalar_safe(sol,'TotalObjective_Yuan');
            end
        end
    end
    if ~any(~isnan(admmObj)), return; end
    fig = create_figure();
    ax = axes(fig);
    bar(ax, 1:nScen, admmObj, 'LineWidth', 1.0, 'FaceColor', [0.00 0.45 0.74]);
    grid(ax,'on');
    ax.XTick=1:nScen; ax.XTickLabel=prettify_scenario_names(scenNames);
    ax.XTickLabelRotation=12; ylabel('Objective value (Yuan)');
    title('Fig. 1. ADMM objective comparison');
    finalize_axes(ax);
...
function draw_cost_breakdown(R, scenName, outDir)
    ...
    ylabel('Cost (Yuan)');
    title(sprintf('Cost breakdown in %s', prettify_one_name(scenName)));
...
function draw_convergence(R, scenName, outDir)
    ...
    title(sprintf('Convergence in %s', prettify_one_name(scenName)));
...
function draw_grid_purchase_combined(allResults, scenNames, outDir)
    ...
    for k = 1:numel(scenNames)
        ...
        if isempty(sol) || ~has_numeric_field(sol,'Pgrid')
            fprintf('Skip grid purchase curve for %s: no solved ADMM Pgrid field.\n', scenNames{k});
            continue;
        end
        y = sum(sol.Pgrid, 1);
        T = numel(y);
    ...
    if ~hasAny
        close(fig);
        fprintf('Skip combined grid purchase plot: no solved ADMM Pgrid fields.\n');
        return;
    end
    ...
    title(ax, 'ADMM grid purchase comparison across scenarios');
...
function draw_renewable_profiles(R, scenName, outDir, caseIdx)
    ...
    xlim(ax, [1 T]);
    yMax = max([pvUse(:); windUse(:); renewableCurt(:)]);
...
function draw_energy_balance(R, scenName, outDir)
    ...
    if isfield(sol,'Heb')
        plot(sum(sol.Heb,1),'-o','LineWidth',1.8,'MarkerSize',4.8, ...
            'MarkerFaceColor','w','DisplayName','E-boiler');
    end
    if isfield(sol,'Hchp')
        plot(sum(sol.Hchp,1),'--s','LineWidth',1.8,'MarkerSize',4.8, ...
            'MarkerFaceColor','w','DisplayName','CHP heat');
    end
    ylabel('Thermal output (MW)'); title('Thermal');
    legend('Location','best'); finalize_axes(gca);
    title(tlo, sprintf('Thermal balance in %s', prettify_one_name(scenName)));
    safeSceneName = strrep(prettify_one_name(scenName),' ','_');
    export_dual(fig, outDir, sprintf('Fig9_Energy_Balance_%s', safeSceneName));
end

function [powerStack, labels, colors] = build_hydrogen_power_stack(sol, D)
    [N, T] = infer_matrix_size(sol, D, {'H2prod','H2dis','H2ch','H2cons_fc','H2short','SOC_h2'}, 'H2load');
    powerStack = zeros(N, T, 0);
    labels = {};
    colors = zeros(0,3);

    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, get_matrix_or_zeros(sol, 'H2prod', N, T), 'H2 produced', [0.00 0.45 0.74]);
    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, get_matrix_or_zeros(sol, 'H2dis', N, T), 'H2 storage discharge', [0.47 0.67 0.19]);
    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, get_matrix_or_zeros(sol, 'H2short', N, T), 'H2 shortage', [0.64 0.08 0.18]);
    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, -get_matrix_or_zeros(D, 'H2load', N, T), 'H2 load', [0.55 0.55 0.55]);
    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, -get_matrix_or_zeros(sol, 'H2cons_fc', N, T), 'H2 to fuel cell', [0.30 0.75 0.93]);
    [powerStack, labels, colors] = add_stack_layer(powerStack, labels, colors, -get_matrix_or_zeros(sol, 'H2ch', N, T), 'H2 storage charge', [0.85 0.33 0.10]);
end

function draw_hydrogen_profiles(R, D, scenName, outDir)
    sol = get_admm_solution(R);
    if isempty(sol) || ~has_numeric_field(sol,'SOC_h2'), return; end
    [powerStackH2, labelsH2, colorsH2] = build_hydrogen_power_stack(sol, D);
    if isempty(labelsH2), return; end
    fig = draw_storage_power_soc_profile(powerStackH2, labelsH2, colorsH2, sol.SOC_h2, ...
        get_field_or_empty(D,'EH2Max'), get_field_or_empty(D,'SOC0_h2'), ...
        sprintf('Hydrogen system in %s', prettify_one_name(scenName)), ...
        'Hydrogen system', 'Hydrogen flow (kg/h)', [0.42 0.10 0.55]);
    safeSceneName = strrep(prettify_one_name(scenName),' ','_');
...
%             'MarkerFaceColor', 'w', 'DisplayName', 'Electric load');
%         h(end+1) = plot(t, D.Hload(i,:), '-s', ...
%             'LineWidth', 1.6, 'MarkerSize', 5.5, ...
%             'MarkerFaceColor', 'w', 'DisplayName', 'Heat load');
% 
%         if isfield(D,'Pwind') && size(D.Pwind,1) >= i
%             h(end+1) = plot(t, D.Pwind(i,:), '-o', ...
%                 'LineWidth', 1.6, 'MarkerSize', 6, ...
%                 'MarkerFaceColor', 'w', 'DisplayName', '椋庣數');
%         end
% 
%         if isfield(D,'Ppv') && size(D.Ppv,1) >= i
%             h(end+1) = plot(t, D.Ppv(i,:), '-d', ...
%                 'LineWidth', 1.6, 'MarkerSize', 5.5, ...
%                 'MarkerFaceColor', 'w', 'DisplayName', '鍏変紡');
%         end
% 
%         xlabel(ax, '鏃堕棿 (h)', 'FontName', fontCN, 'FontSize', 12, 'Interpreter', 'none');
%         ylabel(ax, '鍔熺巼 (MW)', 'FontName', fontCN, 'FontSize', 12, 'Interpreter', 'none');
%         title(ax, sprintf('绀惧尯%d璐熻嵎涓庡彲鍐嶇敓鍑哄姏鏇茬嚎', i), 'FontName', fontCN, 'FontSize', 14, 'Interpreter', 'none');
% 
%         xlim([1, T]);
%         if T <= 24
%             xticks(1:T);
...
function sol = get_admm_solution(R)
    sol = [];
    if nargin < 1 || isempty(R) || ~isstruct(R) || ~isfield(R,'admm')
        return;
    end
    if is_solution_ok(R.admm)
        sol = R.admm;
    end
end
