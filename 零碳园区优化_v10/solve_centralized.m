...
% P1 acceleration: when ADMM optimizer caches are reused across scenarios,
% avoid yalmip('clear') here because it invalidates persistent optimizer handles.
% Standalone centralized runs keep the original clearing behavior by default.
preserveOptimizerCache = isfield(data, 'preserveOptimizerCache') && data.preserveOptimizerCache;
if ~preserveOptimizerCache
    yalmip('clear');
end
...
QtradeMax_tCO2 = i_get_field(data, 'QtradeMax_tCO2', 1.6);
QmarketMax_tCO2 = i_get_field(data, 'QmarketMax_tCO2', 1e4);
carbonBalanceMode = char(i_get_field(data, 'carbonBalanceMode', 'period'));
...
carbonQuotaMode = i_get_field(data, 'carbonQuotaMode', 'baseline-scenario');
fixedCarbonQuota_kg = i_get_field(data, 'fixedCarbonQuota_kg', []);
useFixedCarbonQuota = strcmpi(char(carbonQuotaMode), 'fixed-baseline') && ~isempty(fixedCarbonQuota_kg);
if ~useFixedCarbonQuota
    fixedCarbonQuota_kg = [];
end
...
uHch = sdpvar(N,T,'full');   % 1: charging heat, 0: discharging heat (relaxed)

% Hydrogen storage
H2ch = sdpvar(N,T,'full');
H2dis = sdpvar(N,T,'full');
SOC_h2 = sdpvar(N,T,'full');
uH2ch = sdpvar(N,T,'full');   % 1: charging hydrogen, 0: discharging hydrogen (relaxed)
...
% Raw emission/quota accounting below remains in kgCO2 and is converted in the carbon balance.
...
F = [F, Qwind >= -repmat(data.QwindMax,1,T), Qwind <= repmat(data.QwindMax,1,T)];
...
if isfinite(QtradeMax_tCO2) && QtradeMax_tCO2 > 0
    F = [F, -QtradeMax_tCO2 <= Qtrade, Qtrade <= QtradeMax_tCO2];
end
...
F = [F, H2ch <= repmat(data.H2chMax,1,T).*uH2ch, H2dis <= repmat(data.H2disMax,1,T).*(1-uH2ch)];

% Hydrogen shortage
F = [F, H2short >= 0];
F = [F, H2short <= data.H2load];
...
for i = 1:N
    for t = 1:T
        ...
        if t == 1
            F = [F, SOC_h2(i,t) == data.SOC0_h2(i) + (H2ch(i,t) - H2dis(i,t))*dt];
        else
        ...
        F = [F, PwindUse(i,t)^2 + Qwind(i,t)^2 <= data.QwindMax(i)^2 + 1e-6];
    ...
    if enableCarbonQuota
        if strcmpi(carbonBalanceMode, 'period') || strcmpi(carbonBalanceMode, 'hourly')
            % Period-wise carbon balance. emission_t/quota_t are kgCO2;
            % QaBuy/QaSell/Qtrade/QaUnused are tCO2.
            for t = 1:T
                chpHeatEquivalent_it = ceh*Pchp(i,t) + Hchp(i,t);
                emission_it = zetaE*Pgrid(i,t)*dt + zetaH*chpHeatEquivalent_it*dt;
                if isempty(fixedCarbonQuota_kg)
                    quota_it = chiE*Pgrid(i,t)*dt + chiH*chpHeatEquivalent_it*dt;
                else
                    quota_it = fixedCarbonQuota_kg(i,t);
                end
                F = [F, quota_it/1000 + QaBuy(i,t) + Qtrade(i,t) == ...
            ...
            F = [F, quota_day/1000 + sum(QaBuy(i,:)) + sum(Qtrade(i,:)) == ...
                    emission_day/1000 + sum(QaSell(i,:)) + sum(QaUnused(i,:))];
        end
    end
...
for i = 1:N
    for t = 1:T
        ...
        if enableCarbonQuota
            Obj = Obj + carbonBuyPrice*QaBuy(i,t) - carbonSellPrice*QaSell(i,t);
        end
        ...
        Obj = Obj + cRampEb(i)*(RupEb(i,t) + RdnEb(i,t));
        Obj = Obj + cRampElec(i)*(RupElec(i,t) + RdnElec(i,t));
        Obj = Obj + cRampFc(i)*(RupFc(i,t) + RdnFc(i,t));
        Obj = Obj + cRampComp(i)*(RupComp(i,t) + RdnComp(i,t));
        ...
        Obj = Obj + cDRShiftE(i)*PdrShiftDev(i,t)*dt + cDRCutE(i)*PdrCutE(i,t)*dt;
        Obj = Obj + cDRCutH(i)*HdrCut(i,t)*dt + cDRCutH2(i)*H2drCut(i,t)*dt;
        Obj = Obj + (data.lambdaQpv(i)*(Qpv(i,t)^2) + data.lambdaQwind(i)*(Qwind(i,t)^2) + data.lambdaQes(i)*(Qes(i,t)^2))*dt;
    end
end
...
pvCurtCost = 0; windCurtCost = 0; h2ShortCost = 0; qSupportCost = 0; demandResponseCost = 0;
for i = 1:N
    for t = 1:T
        gridCost      = gridCost      + data.ce(i,t)*Pgrid(i,t)*dt;
        gasCost      = gasCost      + data.cGas*Fgas(i,t)*dt;
        if enableCarbonQuota
            carbonTradingCost = carbonTradingCost + carbonBuyPrice*QaBuy(i,t) - carbonSellPrice*QaSell(i,t);
        end
        ...
        rampCost    = rampCost    + cRampCHP(i)*(RupChp(i,t) + RdnChp(i,t)) + ...
            cRampEb(i)*(RupEb(i,t) + RdnEb(i,t)) + cRampElec(i)*(RupElec(i,t) + RdnElec(i,t)) + ...
            cRampFc(i)*(RupFc(i,t) + RdnFc(i,t)) + cRampComp(i)*(RupComp(i,t) + RdnComp(i,t));
        ...
        demandResponseCost = demandResponseCost + cDRShiftE(i)*PdrShiftDev(i,t)*dt + cDRCutE(i)*PdrCutE(i,t)*dt + ...
            cDRCutH(i)*HdrCut(i,t)*dt + cDRCutH2(i)*H2drCut(i,t)*dt;
        qSupportCost= qSupportCost+ (data.lambdaQpv(i)*Qpv(i,t)^2 + data.lambdaQwind(i)*Qwind(i,t)^2 + data.lambdaQes(i)*Qes(i,t)^2)*dt;
    end
end
res.parts = struct('gridCost', i_value(gridCost), 'carbonCost', i_value(carbonCost), 'gasCost', i_value(gasCost), ...
    ...
    'h2ShortCost', i_value(h2ShortCost), 'demandResponseCost', i_value(demandResponseCost), ...
    'qSupportCost', i_value(qSupportCost));
...
res.RupEb = value(RupEb); res.RdnEb = value(RdnEb);
res.Pelec = value(Pelec); res.H2prod = value(H2prod);
res.RupElec = value(RupElec); res.RdnElec = value(RdnElec);
res.H2cons_fc = value(H2cons_fc); res.Pfc = value(Pfc);
res.RupFc = value(RupFc); res.RdnFc = value(RdnFc);
res.Pcomp = value(Pcomp); res.RupComp = value(RupComp); res.RdnComp = value(RdnComp);
...
res.PdrShift = value(PdrShift); res.PdrShiftDev = value(PdrShiftDev); res.PdrCutE = value(PdrCutE);
res.HdrCut = value(HdrCut); res.H2drCut = value(H2drCut);
res.PloadDR = PloadFixed + res.PdrShift - res.PdrCutE;
res.HloadDR = data.Hload - res.HdrCut;
res.H2loadDR = data.H2load - res.H2drCut;
res.Qpv = value(Qpv); res.Qwind = value(Qwind); res.Qes = value(Qes); res.Qinj = value(Qinj);
...
res.totalPdrShiftDev = sum(res.PdrShiftDev(:))*dt;
res.totalPdrCutE = sum(res.PdrCutE(:))*dt;
res.totalHdrCut = sum(res.HdrCut(:))*dt;
res.totalH2drCut = sum(res.H2drCut(:))*dt;
end
