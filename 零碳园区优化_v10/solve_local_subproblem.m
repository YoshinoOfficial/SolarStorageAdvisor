...
carbonQuotaMode = local_getfield(p,'carbonQuotaMode','baseline-scenario');
carbonBalanceMode = char(local_getfield(p,'carbonBalanceMode','period'));
QtradeMax_tCO2 = local_getfield(p,'QtradeMax_tCO2',1.6);
QmarketMax_tCO2 = local_getfield(p,'QmarketMax_tCO2',1e4);
fixedCarbonQuota_kg = local_getfield(p,'fixedCarbonQuota_kg',[]);
hasFixedCarbonQuota = strcmpi(char(carbonQuotaMode),'fixed-baseline') && ~isempty(fixedCarbonQuota_kg);
...
pfTan = local_getfield(p,'pfTan',0);
cDRShiftE = local_getfield(p,'cDRShiftE',0);
cDRCutE = local_getfield(p,'cDRCutE',0);
cDRCutH = local_getfield(p,'cDRCutH',0);
cDRCutH2 = local_getfield(p,'cDRCutH2',0);
...
key = strjoin({ ...
    ...
    sprintf('cbal=%s',carbonBalanceMode), sprintf('QtrMax=%.10g',QtradeMax_tCO2), sprintf('QmktMax=%.10g',QmarketMax_tCO2), ...
    ...
    sprintf('lH2=%.10g',p.lambdaH2Short), sprintf('QpvM=%.10g',p.QpvMax), sprintf('QwindM=%.10g',p.QwindMax), sprintf('QesM=%.10g',p.QesMax), ...
    sprintf('lQp=%.10g',p.lambdaQpv), sprintf('lQw=%.10g',p.lambdaQwind), sprintf('lQe=%.10g',p.lambdaQes), sprintf('Qcomp=%.10g',p.QcompCoeff), ...
    sprintf('pfTan=%.10g',pfTan), sprintf('cDRs=%.10g',cDRShiftE), sprintf('cDRe=%.10g',cDRCutE), ...
    sprintf('cDRh=%.10g',cDRCutH), sprintf('cDRh2=%.10g',cDRCutH2), ...
...
PloadFixedPar = sdpvar(1,T,'full');
PdrShiftBasePar = sdpvar(1,T,'full');
PdrShiftMaxPar = sdpvar(1,T,'full');
PdrCutEmaxPar = sdpvar(1,T,'full');
HloadPar = sdpvar(1,T,'full');
HdrCutMaxPar = sdpvar(1,T,'full');
H2loadPar = sdpvar(1,T,'full');
H2drCutMaxPar = sdpvar(1,T,'full');
...
paramVars = [cPPar(:); cQPar(:); cCPar(:); PloadPar(:); PloadFixedPar(:); PdrShiftBasePar(:); ...
             PdrShiftMaxPar(:); PdrCutEmaxPar(:); HloadPar(:); HdrCutMaxPar(:); H2loadPar(:); H2drCutMaxPar(:); ...
...
minChpHeatShare = local_getfield(p,'minChpHeatShare',0);
carbonQuotaMode = local_getfield(p,'carbonQuotaMode','baseline-scenario');
hasFixedCarbonQuota = strcmpi(char(carbonQuotaMode),'fixed-baseline') && ~isempty(local_getfield(p,'fixedCarbonQuota_kg',[]));
...
H2short=sdpvar(1,T);
PdrShift=sdpvar(1,T); PdrShiftDev=sdpvar(1,T); PdrCutE=sdpvar(1,T);
HdrCut=sdpvar(1,T); H2drCut=sdpvar(1,T);
uHch=sdpvar(1,T);   % 1: charging heat, 0: discharging heat (relaxed)
uH2ch=sdpvar(1,T);  % 1: charging hydrogen, 0: discharging hydrogen (relaxed)
    Qpv=sdpvar(1,T); Qwind=sdpvar(1,T); Qes=sdpvar(1,T); QinjLocal=sdpvar(1,T); PinjLocal=sdpvar(1,T);
    % Carbon allowance decision variables are in tCO2.
    % Raw carbon accounting parameters/emissions/quotas remain in kgCO2 and are converted in the balance constraint.
...
F = [F, H2ch<=p.H2chMax*uH2ch, H2dis<=p.H2disMax*(1-uH2ch)];
    ...
    F = [F, PdrShift>=0, PdrShift<=PdrShiftMaxPar];
    F = [F, PdrShiftDev>=0, PdrShiftDev>=PdrShift-PdrShiftBasePar, PdrShiftDev>=PdrShiftBasePar-PdrShift];
    F = [F, PdrCutE>=0, PdrCutE<=PdrCutEmaxPar];
    F = [F, HdrCut>=0, HdrCut<=HdrCutMaxPar];
    F = [F, H2drCut>=0, H2drCut<=H2drCutMaxPar];
    F = [F, -p.QpvMax<=Qpv, Qpv<=p.QpvMax, -p.QwindMax<=Qwind, Qwind<=p.QwindMax, -p.QesMax<=Qes, Qes<=p.QesMax];
    F = [F, QaBuy>=0, QaSell>=0, QaUnused>=0];
    QtradeMax_tCO2 = local_getfield(p,'QtradeMax_tCO2',1.6);
    QmarketMax_tCO2 = local_getfield(p,'QmarketMax_tCO2',1e4);
    if isfinite(QtradeMax_tCO2) && QtradeMax_tCO2 > 0
        F = [F, -QtradeMax_tCO2 <= QtradeLocal, QtradeLocal <= QtradeMax_tCO2];
    end
    if isfinite(QmarketMax_tCO2) && QmarketMax_tCO2 > 0
        F = [F, QaBuy <= QmarketMax_tCO2, QaSell <= QmarketMax_tCO2, QaUnused <= QmarketMax_tCO2];
    end
    if ~logical(p.enableCarbonQuota)
        ...
        if ~logical(p.allowCarbonMarketSell)
            ...
            % If market selling is enabled, unused allowance should not be a free slack.
            F = [F, QaUnused==0];
        end
    end
...
for t = 1:T
    ...
    F = [F, H2prod(t)+H2dis(t)+H2short(t)==H2loadPar(t)-H2drCut(t)+H2cons_fc(t)+H2ch(t)];

    F = [F, PpvUse(t)+PpvCurt(t)==PpvPar(t)];
    F = [F, PwindUse(t)+PwindCurt(t)==PwindPar(t)];
    F = [F, Pgrid(t)+PpvUse(t)+PwindUse(t)+Pchp(t)+Pfc(t)+Pdis(t) ...
         == PloadFixedPar(t)+PdrShift(t)-PdrCutE(t)+Peb(t)+Pelec(t)+Pch(t)+Pcomp(t)];
    F = [F, PinjLocal(t)==Pgrid(t)];
    F = [F, QinjLocal(t)==pfTan*(PloadFixedPar(t)+PdrShift(t)-PdrCutE(t))+p.QcompCoeff*Pcomp(t)-Qpv(t)-Qwind(t)-Qes(t)];
    F = [F, PpvUse(t)^2+Qpv(t)^2<=p.QpvMax^2 + 1e-6];
    F = [F, PwindUse(t)^2+Qwind(t)^2<=p.QwindMax^2 + 1e-6];
    F = [F, (Pdis(t)-Pch(t))^2+Qes(t)^2<=p.QesMax^2 + 1e-6];

    F = [F, Hchp(t)+Heb(t)+Hdis(t)==HloadPar(t)-HdrCut(t)+Hch(t)+Hdump(t)];

    if logical(p.enableCarbonQuota)
        Obj = Obj + p.carbonBuyPrice*QaBuy(t) - p.carbonSellPrice*QaSell(t);
    end
    ...
    Obj = Obj + cDRShiftE*PdrShiftDev(t)*p.dt + cDRCutE*PdrCutE(t)*p.dt;
    Obj = Obj + cDRCutH*HdrCut(t)*p.dt + cDRCutH2*H2drCut(t)*p.dt;
    Obj = Obj + (p.lambdaQpv*Qpv(t)^2 + p.lambdaQwind*Qwind(t)^2 + p.lambdaQes*Qes(t)^2)*p.dt;
end

F = [F, sum(PdrShift)*p.dt == sum(PdrShiftBasePar)*p.dt];

if minChpHeatShare > 0
    F = [F, sum(Hchp)*p.dt >= minChpHeatShare * sum(HloadPar-HdrCut)*p.dt];
end

if logical(p.enableCarbonQuota)
    carbonBalanceMode = char(local_getfield(p,'carbonBalanceMode','period'));
    if strcmpi(carbonBalanceMode, 'period') || strcmpi(carbonBalanceMode, 'hourly')
        % Period-wise carbon balance, consistent with the paper formulation.
        % emission_t/quota_t are kgCO2; allowance-trading variables are tCO2.
        for t = 1:T
            chpHeatEquivalent_t = p.ceh*Pchp(t) + Hchp(t);
            emission_t = p.zetaE*Pgrid(t)*p.dt + p.zetaH*chpHeatEquivalent_t*p.dt;
            if ~hasFixedCarbonQuota
                quota_t = p.chiE*Pgrid(t)*p.dt + p.chiH*chpHeatEquivalent_t*p.dt;
            else
                quota_t = fixedCarbonQuotaPar(t);
            end
            F = [F, quota_t/1000 + QaBuy(t) + QtradeLocal(t) == ...
                    emission_t/1000 + QaSell(t) + QaUnused(t)];
        end
    else
        % Optional legacy daily carbon banking mode.
        emission_day = 0;
        quota_day = 0;
        for t = 1:T
            chpHeatEquivalent_t = p.ceh*Pchp(t) + Hchp(t);
        ...
        F = [F, quota_day/1000 + sum(QaBuy) + sum(QtradeLocal) == ...
                emission_day/1000 + sum(QaSell) + sum(QaUnused)];
    end
end
...
for t = 1:T
    if logical(p.enableCarbonQuota)
        Obj_base = Obj_base + p.carbonBuyPrice*QaBuy(t) - p.carbonSellPrice*QaSell(t);
    end
    ...
    Obj_base = Obj_base + cDRShiftE*PdrShiftDev(t)*p.dt + cDRCutE*PdrCutE(t)*p.dt;
    Obj_base = Obj_base + cDRCutH*HdrCut(t)*p.dt + cDRCutH2*H2drCut(t)*p.dt;
    Obj_base = Obj_base + (p.lambdaQpv*Qpv(t)^2 + p.lambdaQwind*Qwind(t)^2 + p.lambdaQes*Qes(t)^2)*p.dt;
end

ops = sdpsettings('solver','gurobi','verbose',0,'warning',0,'cachesolvers',1);
% Guard ADMM from spending unbounded time in one mixed-integer local solve.
ops.gurobi.TimeLimit = local_getfield(p, 'solverTimeLimit', 200);
ops.gurobi.MIPGap = local_getfield(p, 'solverMIPGap', 1e-4);
ops.gurobi.OutputFlag = local_getfield(p, 'solverOutputFlag', 0);
wantedVec = [Pgrid(:);Pch(:);Pdis(:);SOC_e(:);PpvUse(:);PpvCurt(:);PwindUse(:);PwindCurt(:); ...
             ...
             PdrShift(:);PdrShiftDev(:);PdrCutE(:);HdrCut(:);H2drCut(:); ...
             Qpv(:);Qwind(:);Qes(:);QinjLocal(:);PinjLocal(:);QaBuy(:);QaSell(:);QtradeLocal(:);QaUnused(:);Obj_base(:)];
...
cache.ns = struct('nPgrid',T,'nPch',T,'nPdis',T,'nSOC_e',T,'nPpvUse',T,'nPpvCurt',T, ...
    ...
    'nH2short',T,'nPdrShift',T,'nPdrShiftDev',T,'nPdrCutE',T,'nHdrCut',T,'nH2drCut',T, ...
    'nQpv',T,'nQwind',T,'nQes',T,'nQinjLocal',T,'nPinjLocal',T, ...
...
PloadFixed = reshape(local_getfield(p,'PloadFixed',p.Pload),T,1);
PdrShiftBase = reshape(local_getfield(p,'PdrShiftBase',zeros(1,T)),T,1);
PdrShiftMax = reshape(local_getfield(p,'PdrShiftMax',PdrShiftBase.'),T,1);
PdrCutEmax = reshape(local_getfield(p,'PdrCutEmax',zeros(1,T)),T,1);
Hload = reshape(p.Hload(:),T,1);
HdrCutMax = reshape(local_getfield(p,'HdrCutMax',zeros(1,T)),T,1);
H2load = reshape(p.H2load(:),T,1);
H2drCutMax = reshape(local_getfield(p,'H2drCutMax',zeros(1,T)),T,1);
...
carbonQuotaMode = local_getfield(p,'carbonQuotaMode','baseline-scenario');
fixedCarbonQuota_kg = local_getfield(p,'fixedCarbonQuota_kg',[]);
if strcmpi(char(carbonQuotaMode),'fixed-baseline') && ~isempty(fixedCarbonQuota_kg)
    fixedQuota = reshape(fixedCarbonQuota_kg(:),T,1);
else
    fixedQuota = zeros(T,1);
end
paramVec = [cP; cQ; cC; Pload; PloadFixed; PdrShiftBase; PdrShiftMax; PdrCutEmax; ...
            Hload; HdrCutMax; H2load; H2drCutMax; Ppv; Pwind; PcompFixed; ce; fixedQuota];
end
...
[tmp,idx]=local_take(solVec,idx,meta.nPdrShift); out.PdrShift=reshape(tmp,1,T);
[tmp,idx]=local_take(solVec,idx,meta.nPdrShiftDev); out.PdrShiftDev=reshape(tmp,1,T);
[tmp,idx]=local_take(solVec,idx,meta.nPdrCutE); out.PdrCutE=reshape(tmp,1,T);
[tmp,idx]=local_take(solVec,idx,meta.nHdrCut); out.HdrCut=reshape(tmp,1,T);
[tmp,idx]=local_take(solVec,idx,meta.nH2drCut); out.H2drCut=reshape(tmp,1,T);
out.PloadDR=local_getfield(p,'PloadFixed',p.Pload) + out.PdrShift - out.PdrCutE;
out.HloadDR=p.Hload - out.HdrCut;
out.H2loadDR=p.H2load - out.H2drCut;
[tmp,idx]=local_take(solVec,idx,meta.nQpv); out.Qpv=reshape(tmp,1,T);
[tmp,idx]=local_take(solVec,idx,meta.nQwind); out.Qwind=reshape(tmp,1,T);
