...
% 注意：main_year 不再固定使用同一个 ddreScenarioId。
...
% 基准修正：只保留全局负荷修正，不再用比例系数人为制造天气差异。
% 天气差异由不同 1-Day Scenarios/scenario_XXX.csv 的真实日风光曲线体现。
baseScenario = struct('name','BaseDay','name_cn','基准日', ...
    'pvScale',5,'windScale',5,'loadScale',1.0,'h2Scale',1.00,'days',0);

% 典型场景列表：每个天气场景绑定一个独立 ddreScenarioId。
% pvScale/windScale 保持为 1，避免把“天气变化”误写成“装机容量变化”。
...
% Paper-consistent benchmark carbon allowance mechanism.
% Do NOT compute a fixed quota from a no-storage baseline scenario.
% The referenced paper allocates free allowances by the benchmark method:
%   Qa_grid = chiE * Pgrid * dt
%   Qa_CHP  = chiH * (ceh * Pchp + Hchp) * dt
%   Qa_GB   = chiH * H_GB * dt      % omitted here because this code has no GB variable
% Therefore, in this codebase the effective quota is dynamically formed inside
% solve_centralized / solve_local_subproblem as:
%   Qa = chiE*Pgrid*dt + chiH*(ceh*Pchp + Hchp)*dt.
% Keep the variable below only for save-file backward compatibility.
...
for k = 1:numel(typicalScenarios)
    ...
    % 关键修改：每个典型天气场景读取不同的 1-Day Scenario。
    % 这样晴天/阴天、少风/多风不仅是幅值不同，而且时序波动形态也不同。
    ...
    % Paper benchmark method: free quota is not fixed before optimization.
    % It is calculated endogenously from current dispatch variables using
    % chiE, chiH and ceh inside the centralized and local solvers.
    ...
    scenarioInfoRows = [scenarioInfoRows; make_scenario_info_row(data)]; %#ok<AGROW>
        ...
        fprintf('Case: %s | DDRE-33 day scenario: %03d | pv_label=%d | wind_label=%d\n', ...
            caseName, data.ddreScenarioId, data.pvLabel, data.windLabel);
...
%DEFINE_TYPICAL_SCENARIOS 基于 DDRE 1-Day Scenarios 的典型天气/新能源场景。
%
% 标签来自 1-Day Scenarios/scenario_labels.csv：
%   pv_label   = 0/1/2 分别表示高光伏(晴天)、中光伏(多云)、低光伏(阴天/雨天)
%   wind_label = 0/1/2 分别表示少风、中风、多风
%
% 本版本不再用 pvScale/windScale 人为放大同一条风光曲线。
% 每个典型场景直接绑定一个 scenario_XXX.csv，保持真实的日内波动形态。
%
% representative days 说明：
%   days 按所选 5 类标签组合在 200 个样本中的数量比例折算，合计 365 天。
%   如需与某地真实气象统计匹配，可只改 days，不影响场景曲线读取逻辑。
%
% 代表日选择规则：
%   在对应 (pv_label, wind_label) 标签组内，选择日均光伏和日均风电
%   最接近该组中位水平的 scenario_XXX.csv。这样避免选到极端样本。
...
if isfield(data, 'ddreScenarioId')
    row.DDREScenarioId = data.ddreScenarioId;
else
    row.DDREScenarioId = NaN;
end
if isfield(data, 'pvLabel')
    row.PVLabel = data.pvLabel;
else
    row.PVLabel = NaN;
end
if isfield(data, 'windLabel')
    row.WindLabel = data.windLabel;
else
    row.WindLabel = NaN;
end
if isfield(data, 'ddreSourceCount')
    row.DDRESourceCount = data.ddreSourceCount;
else
    row.DDRESourceCount = NaN;
end
if isfield(data, 'ddreSelectionRule')
    row.DDRESelectionRule = string(data.ddreSelectionRule);
else
    row.DDRESelectionRule = "";
end
...
%GET_NUMERIC_DATA_FIELD Safe numeric metadata getter for exported result rows.
