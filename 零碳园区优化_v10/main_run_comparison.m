...
runCentralized = 1;
runFixedADMM = 1;
admmRhoPQ = 30;    % P/Q feeder-consensus ADMM penalty
...
for k = 1:numel(scenarios)
    ...
    fprintf('PV scale=%.3f, Wind scale=%.3f, Storage=%d, CarbonTrading=%d, rhoPQ=%.3f, rhoC=%.3f\n', ...
        data.pvScaleFinal, data.windScaleFinal, data.storageEnabled, data.enableCarbonTrading, data.admmRhoPQ, data.admmRhoC);
    fprintf('============================================================\n');
...
try
    writetable(summaryTable, 'comparison_summary.csv');
    writetable(metricTable, 'comparison_metric_table.csv');
catch ME
...
scenarios = struct( ...
    ...
    'pvScale', {5.5, 5.5, 5.5, 5.7}, ...
    'windScale', {5.5, 5.5, 5.5, 5.7}, ...
...
keep = {'scenarioName','scenarioNameCN','comparisonScenario','comparisonScenarioCN', ...
    ...
    'N','T','dt','Pload','Hload','H2load','Ppv','Pwind','PcompFixed','alphaCompH2','PcompMax','ce','PchpRated','etaE_chp','etaH_chp', ...
    'minChpHeatShare', ...
